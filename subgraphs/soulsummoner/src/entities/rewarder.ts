...
import { ComplexRewarder as ComplexRewarderContract } from '../../generated/SoulSummoner/ComplexRewarder'
import { ConvexRewarder as ConvexRewarderContract } from '../../generated/SoulSummoner/ConvexRewarder'
...
export function getRewarder(address: Address, block: ethereum.Block): Rewarder {
  ...
  if (rewarder === null) {
    ...
    if (CONVEX_REWARDERS.includes(address)) {
      rewarder.rewardToken = Address.fromString('0x4e3fbd56cd56c3e72c1403e103b45db9da5b9d2b') // todo: update
    }
    else if (address == ALCX_REWARDER) {
      rewarder.rewardToken = Address.fromString('0xdbdb4d16eda451d0503b854cf79d55697f90c8df') // todo: update
    }
