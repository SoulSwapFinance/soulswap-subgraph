...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = SoulSwapFactory.load(getFactoryAddress())
...
export function handleSync(event: Sync): void {
  ...
  let soulswap = SoulSwapFactory.load(getFactoryAddress())

  // reset factory liquidity by subtracting onluy tarcked liquidity
  soulswap.totalLiquidityNativeCurrency = soulswap.totalLiquidityNativeCurrency.minus(
  ...
  soulswap.totalLiquidityNativeCurrency = soulswap.totalLiquidityNativeCurrency.plus(trackedLiquidityNativeCurrency)
  soulswap.totalLiquidityUSD = soulswap.totalLiquidityNativeCurrency.times(bundle.nativeCurrencyPrice)
...
export function handleSwap(event: Swap): void {
  ...
  soulswap.totalVolumeNativeCurrency = soulswap.totalVolumeNativeCurrency.plus(trackedAmountNativeCurrency)
  ...
  let soulswapDayData = updateSwapDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  soulswapDayData.dailyVolumeUSD = soulswapDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  soulswapDayData.dailyVolumeNativeCurrency = soulswapDayData.dailyVolumeNativeCurrency.plus(trackedAmountNativeCurrency)
