...
import {
  ...
  getFusdNativeCurrencyWrapperPair,
...
export function getNativeCurrencyPriceInUSD(): BigDecimal {
  ...
  let fusdPair = Pair.load(getFusdNativeCurrencyWrapperPair()) // fusd is token1

  // all 3 have been created
  if (daiPair !== null && usdcPair !== null && fusdPair !== null) {
    let totalLiquidityNativeCurrency = daiPair.reserve1.plus(usdcPair.reserve1).plus(fusdPair.reserve0)
    ...
    let fusdWeight = fusdPair.reserve0.div(totalLiquidityNativeCurrency)
    ...
    let fusdPairPrice = fusdPair.token0 == nativeCurrencyWrapperAddress ? fusdPair.token1Price : fusdPair.token0Price

    return daiPairPrice
      .times(daiWeight)
      .plus(usdcPairPrice.times(usdcWeight))
      .plus(fusdPairPrice.times(fusdWeight))
